...
2. 使用平时上课的 Windows 账户双击 initialize.cmd。
   输入两次至少 6 位的网页登录密码；四个页面共用一个密码，平板登录后会记住。遇到防火墙管理员授权时同意。
...
3. 必须看到所有步骤 OK，四个服务通过健康和网页检查；FAILED 或 NOT COMPLETE 都不是成功。
   ...
   防火墙规则适用于域/专用/公用配置，但只允许当前本地子网，避免网络被识别为“公用”时平板完全断开。
4. 平板与教学机连接同一局域网，用窗口列出的教学机 IPv4 地址访问：
   7000：课件、截图、下载、投放、剪贴板。
         “教学控制”栏还可操作音量、媒体播放、PowerPoint 放映/翻页、桌面和任务视图。
...
6. 正式上课前重启一次，用平板再次访问四个端口。
   ...
   如果 PowerPoint 结束放映后触控板失控，可在 7000 页面右上角点击“重启触控板”，等待约 10 秒后刷新 7050。

无需安装 Python。随包包含 Claude Code x64、CC Switch、Portable Git/Bash，
以及微软 WebView2 离线安装程序。初始化无需临时下载这些依赖。
服务默认在用户登录后隐藏启动；启动文件夹、当前用户 Run 注册表和延迟 15 秒的计划任务提供三重入口，监督器每 8 秒检查并重启异常服务。
计划任务不依赖 Explorer，适用于畅言等替换 Windows 桌面的教学系统；三个入口由独占锁合并为一个监督器。
它不是锁屏/未登录桌面的 Windows 系统服务；必须先登录上课账户。
不要删除 _internal 文件夹。不要在初始化后移动程序目录；若移动，重新初始化。

数据、配置和日志
%LOCALAPPDATA%\TeachingGateway
  auth.json：四服务共用的网页登录密码哈希和会话密钥，禁止外传。
...
自动启动异常：用同一个上课账户重新 initialize.cmd；它会修复 TeachingGateway.lnk 和当前用户 Run 项。
停止：stop.cmd，停止本目录服务和当前监督器，保留数据；下次登录仍会自启动。
